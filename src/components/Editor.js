...
const [highScore, setHighScore] = useState(null);
    ...
    const fetchHighScore = async () => {
        try {
            //const response = await fetch('http://127.0.0.1/getHighScore', {
            const response = await fetch('https://cs218-finalbackend.vercel.app/getHighScore', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ user: user.user.username, questionName: "KnightAttack" })
            });
    ...
    const handleSubmit = async (code, user) => {
        ...
        //const response = await fetch('http://127.0.0.1/runCode', {
